# 简易教程

> **SR6/OSR6 Realtime Screen TCode High Hardware Compatibility v2.0.0-test.3：** BETA 预发布提供 Windows 免安装和源码 ZIP。完整解压后双击 `Start.cmd`；免安装版无需另装 Python。模型与可选 GPU 运行库在软件内下载，不随包分发。测量与原有测试动作针对原生 TCode 设备，新增硬件与自定义绑定请看[设备兼容说明](Device_Compatibility_2.0.md)，不保证兼容效果。GPU 一键安装等最新说明见[主 README](../README.md)。

**成年人使用提示：本项目面向成年人使用，未成年禁止入内。**

**重要警告：六轴模式只建议在光线好、主体清晰、框选区域干净的画面里使用。** 如果画面不清楚，请先用 L0 模式或 Log only 预览。

第一次正式使用前，建议先用“测量模式”保存适合你自己设备和体感的上限/下限。上下限会保存到本地电脑。下次打开如果能正常输出脚本，就不用再点“恢复所有默认设置”，除非你想全部重置。

提示：鼠标光标悬浮在按钮、滑块、参数名上，会出现小字解释，告诉你这个功能是干嘛的。

## 下载

Windows 免安装包请先完整解压文件夹，再运行 `SR6-OSR6-Realtime-Screen-TCode-High-Hardware-Compatibility.exe` 或 `Start.cmd`。

如果是源码用户，clone 仓库后双击 `Start.cmd` 或 `Start-Source.cmd`。

## 有 OSR6 设备时

1. 优先双击 `SR6-OSR6-Realtime-Screen-TCode-High-Hardware-Compatibility.exe`，或双击 `Start.cmd`。

2. 先阅读成年人警告；只有确认自己已成年才继续。

![第 2 步 成年人警告](../screenshots/guide-02-age-warning.png)

3. 在语言弹窗里选择 `中文`。

![第 3 步 选择语言](../screenshots/guide-03-language.png)

4. 第一次打开，先点“恢复所有默认设置”，确认一次。

![第 4 步 恢复默认](../screenshots/guide-zh-04-restore-defaults.png)

5. 在左侧连接区域选择 USB 串口或 BLE。

![第 5 步 选择输出方式](../screenshots/guide-zh-05-output-type.png)

6. 点“连接并回中”，确认软件显示已连接。

![第 6 步 连接并回中](../screenshots/guide-zh-06-connect-center.png)

7. 点“测量模式”，用滑块遥控 L0 上下移动。

![第 7 步 测量模式](../screenshots/guide-zh-07-measurement-mode.png)

8. 保存一个舒服、安全的上限和下限。

![第 8 步 保存上下限](../screenshots/guide-zh-08-save-limits.png)

9. 先用第 `3` 档。

![第 9 步 第三档预设](../screenshots/guide-zh-09-preset-3.png)

10. 点“开始实时输出”。

![第 10 步 开始实时输出](../screenshots/guide-zh-10-start-realtime.png)

11. 弹窗里建议先选 `L0 Only`；六轴只在光线好、主体清晰、框选干净时再开。

![第 11 步 输出模式](../screenshots/guide-zh-11-output-mode.png)

12. 确认或重新框选屏幕区域，并优先使用“混合分析（推荐-非舞蹈）”。

![第 12 步 框选和分析模式](../screenshots/guide-zh-12-region-analysis.png)

13. 点“确认开始”。稳定后再慢慢加刺激度。

![第 13 步 确认开始](../screenshots/guide-zh-13-confirm-start.png)

上下限和设置会保存到本地。下次如果能正常输出脚本，就不用再点“恢复所有默认设置”。

## 没有机器也想先看预览

可以先不连接设备，安全看一下软件会怎么分析画面。

1. 打开软件，确认成年人提示，并选择中文。
2. 输出选择 `Log only`。
3. 点“显示预览”。
4. 屏幕上打开一个有运动的视频或网页。
5. 点“开始实时输出”。
6. 弹窗里先选择 `L0 Only`。
7. 框选正在动的画面区域。
8. 看曲线和预览变化。Log only 模式只显示输出，不会控制硬件。

想看所有面板和参数的详细说明，请读 `用户使用手册.md`。英文版是 `User_Manual_EN.md`。

## 联系

侵权、许可证修正、建议、合作反馈请联系：aivnailedeng@gmail.com

Discord 交流：https://discord.gg/E7RY3rdKw

Discord 提醒：如果进入交流群，请遵守成年人限制。服务器/相关频道建议设置为年龄受限，未成年禁止进入或查看相关内容。
